...
const StashPhotosBottomSheet = forwardRef(
  (
    {
      sheetRef,
      snapPoints,
    ...
    ref // This `ref` is the ref forwarded from the parent component
  ) => {
    ...
    // 🔥 Expose the cancel function via the ref
    ...
    const uploadFile = async (url, uri, mimeType = "image/jpeg", signal) => {
      ...
      while (true) {
        ...
        try {
          ...
          if (signal.aborted) {
            console.warn("⚠️ Upload aborted inside catch");
            throw new DOMException("Upload aborted", "AbortError");
          }
          ...
          if (
            ...
            throw err; // ❌ User manually cancelled
          }

          console.warn("🌐 Network issue detected. Waiting to retry upload...");
          await waitForNetwork(); // ⏳ wait until network is back
      ...
      try {
        ...
        for (let i = 0; i < selectedPhotos.length; i++) {
          ...
          try {
            ...
            if (err.name === "AbortError" || compressionCancelled.current)
              break;
          }
        }
        ...
        if (compressionCancelled.current)
          throw new Error("Confirmation skipped");
        if (uploadedInfo.length > 0) {
          ...
          Alert.alert("Stash failed", "No photos were stashed successfully.");
        }
    ...
    return (
      <>
        {isOpen && (
          ...
          >
            <View
              style={{ flex: 1, backgroundColor: grey2, paddingBottom: 50 }}
            >
...
export default StashPhotosBottomSheet;
